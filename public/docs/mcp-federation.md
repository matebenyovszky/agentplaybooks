...
`access: "public"` allows anyone who can access the public playbook to incur upstream calls. Prefer `playbook_api_key`; callers then need a playbook key with `tools:call` or `full` permission.
