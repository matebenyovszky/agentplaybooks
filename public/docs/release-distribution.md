...
| Hermes plugin catalog | Keep the [merged Hermes catalog entry](https://github.com/NousResearch/hermes-agent/pull/119450), pinned source, and native memory provider compatible. Update the pin through an upstream PR for a new plugin revision. | Run `hermes memory status` and a write/read/correction test on a private playbook. |
