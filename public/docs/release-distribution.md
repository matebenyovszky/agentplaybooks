...
| Hermes plugin catalog | Keep the [merged memory catalog entry](https://github.com/NousResearch/hermes-agent/pull/119450), its source pin, and the separate tools plugin compatible. Update catalog pins through upstream PRs when those packages change. | Check both install names, the memory setup, and the tools skill's authentication and profile instructions. |
