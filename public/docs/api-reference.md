...
**Note:** AgentPlaybooks uses **1 playbook = 1 persona**. For backward compatibility, some endpoints still expose `GET /personas` which returns an array (length 1).
...
## Persona (Embedded in Playbook)

**Architecture: 1 Playbook = 1 Persona**

Each playbook has exactly one persona embedded directly in the playbook record. This simplifies the architecture and ensures consistent behavior.

### Get Persona

The persona is returned directly with the playbook:
...
For backward compatibility with existing integrations, a `/personas` endpoint is still available but returns a single-element array:
...
Update the persona via playbook update:
...

{
  ...
  "persona_system_prompt": "Updated prompt...",
  "persona_metadata": { "avatar": "🤖" }
}
```

Or via the legacy persona endpoint (updates the playbook's embedded persona):

```http
PUT /api/playbooks/:id/personas/:id
Content-Type: application/json
...
**Note:** Persona cannot be deleted separately - it's part of the playbook. Clear the values to reset to defaults.
...
| `PUT /api/manage/playbooks/:id` | Update playbook (incl. persona fields) |
...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| `GET` | `/api/playbooks/:id/personas` | JWT/Public* | Get persona (singleton array) |
| `PUT` | `/api/playbooks/:id/personas/:pid` | JWT | Update embedded persona |
