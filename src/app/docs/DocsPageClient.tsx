...
function parseMarkdown(md: string): string {
  ...
  let html = textWithPlaceholders
    // Headers
    .replace(/^#### (.*$)/gm, '<h4 id="$1" class="text-lg font-semibold mt-6 mb-2 text-neutral-800 dark:text-neutral-200">$1</h4>')
    ...
    .replace(/^# (.*$)/gm, '<h1 id="$1" class="text-4xl font-bold mt-6 mb-6 text-neutral-900 dark:text-white">$1</h1>')
    // Bold and italic
    ...
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_, text, url) => {
      ...
      return `<a href="${href}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 underline underline-offset-2">${text}</a>`;
    })
    // Lists
    ...
    .replace(/\|(.+)\|/g, (match) => {
      ...
      const cellClass = isHeader
        ? 'px-4 py-3 text-left text-sm font-semibold text-neutral-800 dark:text-neutral-200 bg-neutral-100 dark:bg-neutral-900'
