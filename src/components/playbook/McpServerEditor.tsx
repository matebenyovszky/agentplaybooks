...
export function McpServerEditor({ mcpServer, storage, onUpdate, onDelete, readOnly = false }: McpServerEditorProps) {
  ...
  const [transportConfigJson, setTransportConfigJson] = useState(
    JSON.stringify(mcpServer.transport_config || { url: "", timeout_ms: 15000, access: "playbook_api_key" }, null, 2)
  );
  ...
  useEffect(() => {
    const changed =
      ...
      transportConfigJson !== JSON.stringify(mcpServer.transport_config || { url: "", timeout_ms: 15000, access: "playbook_api_key" }, null, 2) ||
      secretsChanged;
